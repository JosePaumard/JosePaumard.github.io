...
public class WordWrapStepDefs {
    ...
    @Then("^the result is:$")
    public void the_result_is_hello(List<String> expectedLines) throws Throwable {
        List<String> lines = Pattern.compile("\n").splitAsStream(this.result).collect(Collectors.toList());
        Assertions.assertThat(lines).isEqualTo(expectedLines);
    }
}
